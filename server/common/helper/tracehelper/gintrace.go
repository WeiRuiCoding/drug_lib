...
const (
	tracerKey  = "jz-tracer"
	tracerName = "ctx-tracer"
)

// GinMiddleware returns middleware that will trace incoming requests.
...
func GinMiddleware(service string, opts ...Option) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		sc := span.SpanContext()
		if sc.HasTraceID() {
			c.Header(TraceIdKey, sc.TraceID().String())
		} else {
			c.Header(TraceIdKey, "not-trace")
		}
